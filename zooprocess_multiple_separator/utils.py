...
def predict_panoptic_masks(image_path, model, processor, device,
                           min_mask_score=0.9, bottom_crop=31,
                           max_prop_missing=0.2):
    """
    ...
    threshold, combines all masks and computes a distance map from the mask
    border, compute the center point of ach mask. Finally it returns variables
    relevant for the next part of the process.
    ...
    Args:
        ...
        bottom_crop (int): number of pixels to crop from the bottom of the
          image (e.g. to remove the scale bar for example)
        max_prop_missing (float, in [0,1]): proportion of the area of the
          original object. When the panoptic model's masks do not cover a
          region of the object that is larger than this proportion, consider
          it as a potential new object for the watershed etc. procedure. This
          allows to catch back objects of interest that may have been missing
          by the panoptic segmenter.
    ...
    selected_masks_ids = [seg["id"] for seg in results["segments_info"] if
                          seg["label_id"] == 1 and
                          seg["score"] > min_mask_score]
    # NB: label_id == 1 for objects, 0 for background

    # compute their average score, as an indication of the quality
    # of the segmentation
    ...
    for i in np.delete(missing_regions_ids, 0):
        ...
        if nb_pixels[i] > (total_pixels * max_prop_missing):
            ...
            panoptic_masks[missing_regions == missing_regions_ids[i]] = \
                max_mask_id
...
def separate_with_watershed(panoptic_masks, overall_mask):
    ...
    Args:
        panoptic_masks (np.ndarray): array of masks labels output by
          `predict_panoptic_masks()`. 0 is the background, the other values
          are masks.
    ...
    Returns:
        (np.ndarray) with 0 as background and 1 for lines separating two
        objects.
    """
    ...
    for mask_id in mask_ids:
        ...
        center_coords = np.unravel_index(
    ...
    # Apply watershed
    labels = watershed(
        -dist_map, watershed_markers, mask=overall_mask,
        watershed_line=False, connectivity=2
    )
